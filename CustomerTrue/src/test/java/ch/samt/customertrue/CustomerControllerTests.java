...
import ch.samt.customertrue.service.MealGroupService;
...
public class CustomerControllerTests {
    ...
    public void testLoadMealGroups() throws Exception {
        mockMvc.perform(get("/customers/mealgroups"))
                ...
                .andExpect(view().name("mealGroupsList"))
                .andExpect(model().attribute("mealgroups", mealGroupService.findAll()))
                .andExpect(model().attribute("mealgroups", hasSize(3)))
                .andExpect(model().attribute("mealgroups", hasItem(hasProperty("name", is("Colazione")))))
                .andExpect(model().attribute("mealgroups", hasItem(hasProperty("customers", hasItem(hasProperty("name", is("Guido")))))))
                .andExpect(model().attribute("mealgroups", hasItem(hasProperty("customers", hasItem(hasProperty("surname", is("Bianchi")))))));
    }
}